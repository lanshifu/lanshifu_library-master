...
public class MyCallBack extends ItemTouchHelper.Callback {
    ...
    private List<ImageItem> originImages;//图片没有经过处理，这里传这个进来是为了使原图片的顺序与拖拽顺序保持一致
    private boolean up;//手指抬起标记位

    public MyCallBack(CommonAdapter adapter, List<ImageItem> images, List<ImageItem> originImages) {
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (dY >= (recyclerView.getHeight()
                ...
                - MyApp.getContext().getResources().getDimensionPixelSize(R.dimen.bt_commit))) {//拖到删除处
